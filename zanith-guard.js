/**
 * ============================================================
 * ZANITH ADMIN — PASSWORD GUARD (tự inject, không sửa HTML)
 * ============================================================
 * Cách dùng: Thêm 1 dòng vào <head> của admin.html:
 *   <script src="zanith-guard.js"></script>
 *
...
(function () {
  ...
  #zlock-btn {
    ...
    letter-spacing: 0.2em; text-transform: uppercase; cursor: pointer; transition: background 0.2s, transform 0.1s;
  }
...
<div id="zanith-sync-badge" style="display:none" title="Click để sync ngay">
  ...
  <span id="zanith-sync-text">Chưa sync</span>
    ...
    function tryUnlock() {
      ...
      if (val === ZANITH_ADMIN_PASSWORD) {
        attempts = 0; updateDots(0, false);
        ...
        if (attempts >= MAX_ATTEMPTS) {
          ...
          setTimeout(function() {
            attempts = 0; updateDots(0, false);
            setMsg('', ''); loginBtn.disabled = false;
          }, LOCKOUT_MS);
        } else {
    ...
    function callSheet(action, payload) {
      var body = Object.assign({ secret: ZANITH_SHEET_SECRET, action: action }, payload);
    ...
    function loadDataFromSheet() {
      if (!ZANITH_SHEET_URL || ZANITH_SHEET_URL === 'YOUR_APPS_SCRIPT_URL') {
        setSyncStatus('error', 'Chưa cài URL Script');
        return;
      }
      setSyncStatus('syncing', 'Đang tải...');
      callSheet('getData', {}).then(function(result) {
    ...
    function saveDataToSheet() {
      if (!ZANITH_SHEET_URL || ZANITH_SHEET_URL === 'YOUR_APPS_SCRIPT_URL') return;
      ...
      callSheet('setData', { data: data }).then(function() {
        var d = new Date();
    ...
    // Auto-sync khi localStorage thay đổi
    ...
    window.ZanithSync = { save: saveDataToSheet, load: loadDataFromSheet };
  }
